...
function fetchProfile(setState, data) {
  ...
  const url = `${baseUrl}${config.ticketCancelList}${data.phoneNumber}/${
    data.ticket
  }`;
  Axios.get(url)
    .then(res => {
      setState(res.data.data);
      showToastrOnSuccess('Cancel Tickets fetch Successfully');
    })
    .catch(res => {
        ...
        <Row>
          <div className="custheader">
            ...
            <p style={{ color: 'gray' }}>
              ...
              <span style={{ color: '#da4d52' }}>Cancel</span> your tickets
            </p>
          </div>
